...
/**
 * Class PDF
 * @ORM\Entity(repositoryClass="MainBundle\Repository\PDFRepository")
 * @ORM\Table(name="PDF")
 * 
 */
class PDF {
    ...
    /**
     * @Assert\File(mimeTypes={"application/pdf", "application/x-pdf"},  mimeTypesMessage = "Veuiller insérer un fichier PDF valide", maxSize = "2M", maxSizeMessage = "Le fichier PDF est trop gros ({{ size }} {{ suffix }}) la taille maximum est de {{ limit }} {{ suffix }}.", disallowEmptyMessage = "Un fichier vide n'est pas autorisée.", notFoundMessage = "Le fichier n'a pas pu être trouvée.", notReadableMessage = "Le fichier n'est pas lisable.", uploadIniSizeErrorMessage = "Le fichier est trop gros la taille maximum est de {{ limit }} {{ suffix }}.", uploadFormSizeErrorMessage = "Le fichier est trop gros !", uploadErrorMessage = "Le fichier n'a pas pu être uploadée !")
     */
    private $temporary_path;

    /**
     * @ORM\OneToOne(targetEntity="Projet", inversedBy="pdf")
     * @ORM\JoinColumn(name="projet_id", referencedColumnName="id", nullable=false)
     * @Assert\NotNull(message="Un PDF doit avoir un projet !")
     */
    private $projet;
